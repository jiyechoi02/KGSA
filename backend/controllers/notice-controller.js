...
const createPost = async (req,res) =>{
    ...
    if(files.length != 0){
        ...
        .then(result=>{
            ...
            const notice = new Notice(obj)

            if(!notice){
                return res.status(500).json({
                    success: false,
                    message : "Failed to create new notice Schema",
                })
            }
...
const findByUser = async (req,res) =>{
    ...
    try{
        const result = await Notice.find({username : username }).sort({"createdAt":-1})
        return res.status(200).json({
            success: true,
            data :result
        })
...
const findOnePost = async (req,res)=>{
    ...
    try{
        const result = await Notice.findById({_id : ObjectId(_id)})
...
const updatePost = async (req,res)=>{
    ...
    try{
        ...
        if(files.length != 0){
            ...
            .then(async()=>{
                const result = await Notice.findOneAndUpdate({_id: ObjectId(_id)}, req.body);
...
const deletePost = async (req,res)=>{
    ...
    Notice.findOneAndDelete({_id: ObjectId(_id), username : username})
    .then(result=>{

        const comments = result.comments
        const fileKeys = result.keys? result.key : [];
...
const findAllComments = async (req,res)=>{
    ...
    try{
        const result = await Notice.findById({_id : ObjectId(_id)}).populate('comments')
        const comments = result.comments
...
const deleteComment = async (req,res)=>{
    ...
    try{
        ...
        if(result){
            try{
                const notice = await Notice.findById({_id : ObjectId(_id)})
                notice.comments.pull(_cid)
...
const updateViews = async (req,res)=>{
    const _id = req.params._id;
    try{
        const result = await Notice.findOneAndUpdate({_id : ObjectId(_id)}, {$inc: {'views' :1}})
        return res.status(200).json({
            success : true,
            data : result
        })
...
const updateLikes = async (req,res)=>{
    ...
    try{
        const result = await Notice.findOneAndUpdate({_id : ObjectId(_id)}, {$inc: {'likes' :value }})
        return res.status(200).json({
            success : true,
            data : result
        })
